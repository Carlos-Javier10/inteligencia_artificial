...
X = np.append(arr=np.ones((50,1)).astype(int), values=X, axis=1)  # Añadimos una columna de unos para la constante
...
plt.scatter(X_train, y_train, color="red")  # Datos reales (entrenamiento)
plt.plot(X_train, regression.predict(X_train), color="blue")  # Línea de predicción
plt.title("Sueldo vs Años de Experiencia (Entrenamiento)")
plt.xlabel("Años de Experiencia")
plt.ylabel("Sueldo ($)")
plt.show()

plt.scatter(X_test, y_test, color="red")  # Datos reales (prueba)
plt.plot(X_test, regression.predict(X_test), color="blue")  # Línea de predicción
